...
var sportName = ["Baseball", "BasketBall", "Gymnastics", "Track", "Swimming", "Water Polo", "Opening Ceremony"];
	...
	$(function() {	
		$.getJSON( "https://spreadsheets.google.com/feeds/list/1gJQIn0TvEJ0c-R7Csntfwmu3qwTzqAYJEImdZ720jeQ/od6/public/values?alt=json", function (data) {	
			$.each(data.feed.entry.reverse(), function(i,entry) {
				...
				for (var i = sportName.length - 1; i >= 0; i--) {
					var newLink = ' <li><a href="#anchor'+i+'">'+sportName[i]+'</a></li>';
				...
				if(i==0)
				{
					...
					var newLink = ' <li><a href="#anchor'+i+'">'+sportName[k]+'</a></li>'
					$('ul.nav').append(newLink); 
					cutoff=new Date(sportName[j]);
					j++;k++;
				}*/
